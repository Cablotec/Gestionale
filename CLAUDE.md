...
- **Cache**: a ogni deploy bump `?v=YYYY-MM-DD.N` nei 4 gusci. Attuale: `v=2026-07-15.1`. **Versione visibile sotto il logo** (gestionale e kiosk): prima verifica quando "non si vede una modifica".
...
- **Regole per-cliente = DATI d'anagrafica azienda, mai hardcode**: `tariffa_cliente` €/h = prezzo solo manodopera → nei nuovi ordini tempo pagato = prezzo ÷ tariffa × 60 (vince sul default articolo, toast dichiara). In modifica MAI automatica: suggerimento "da prezzo" + usa. Elcotec 27,3 €/h.
