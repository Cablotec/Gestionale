...
- **Cache**: a ogni deploy bump `?v=YYYY-MM-DD.N` nei 4 gusci. Attuale: `v=2026-08-07.6`. La **versione è visibile sotto il logo** (gestionale e kiosk): prima cosa da controllare quando "non si vede una modifica" (quasi sempre è cache).
...
- **Compare SOLO quando c'è qualcosa**, e si può chiudere per la giornata (`localStorage 'sospette-chiuse'`). Aperto/chiuso dell'elenco vive in `state.sospetteAperto`: il riquadro si ridisegna da solo **una volta al minuto** (non a ogni secondo come il vecchio banner: sarebbe sprecato) e senza quello si richiuderebbe sotto le dita mentre lo si legge.
