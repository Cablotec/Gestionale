...
**Le regole stanno in `analizzaImportOrdini(righe, ctx)` in `domain/scheduling.js`** — pura, nessun DOM, nessun Supabase, **110 test in `scratchpad/test_import_ordini.js`**. Ritorna il PIANO (nuove / aggiornamenti / bloccate / scartate / anagrafiche da creare) e non scrive niente: la UI disegna soltanto.
...
- 110 test in `scratchpad/test_import_ordini.js`, con tutte le regressioni dentro.
...
- **IL CODICE ARTICOLO DEVE ESSERCI SEMPRE** (25 ago, regola di Nico): se manca non e una riga che non ci riguarda, e un **errore nell anagrafica di Alnus** da correggere la. Prima finiva fra gli scarti generici in fondo all anteprima; ora ha un riquadro rosso suo con ordine, cliente, descrizione e quantita — quanto basta per andarla a cercare. Oggi sono le 4 righe di `2026/OC/00393` (Capirossi): minuteria e una riga MANODOPERA.
