...
- ⚠ **LA PISTA VERA: la GRANA dei codici non coincide.** Le 22 scoperte Senzani sono la famiglia `SZ-T127xxBM` / `SZ-T127xxQE` (verosimilmente due assiemi per macchina). Nel file esistono 13 padri `SZ-T127*`, ma sono i **particolari** — `SZ-T12704002002` CASSETTA, `SZ-T12704003002` SCATOLA, `SZ-T12704004028` PIASTRA — non gli assiemi BM/QE, che come padri **non esistono con nessuna grafia**. Alnus tiene le distinte a livello di PARTICOLARE, il gestionale lavora a livello di ASSIEME. **Domanda da fare a chi tiene Alnus**, non deducibile dai dati.
