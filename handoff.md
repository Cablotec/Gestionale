...
**Le regole stanno in `analizzaImportOrdini(righe, ctx)` in `domain/scheduling.js`** — pura, nessun DOM, nessun Supabase, **96 test in `scratchpad/test_import_ordini.js`**. Ritorna il PIANO (nuove / aggiornamenti / bloccate / scartate / anagrafiche da creare) e non scrive niente: la UI disegna soltanto.
...
- 96 test in `scratchpad/test_import_ordini.js`, con tutte le regressioni dentro.
